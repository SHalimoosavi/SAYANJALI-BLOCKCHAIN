...
| TM-012 | Cross-network replay | Funds and consensus | Valid message/transaction copied to another network | Replay signed payload on different chain | Network/chain identity binding, domain separation, versioning | Wrong-network validation failures | Differential replay tests | IMPLEMENTED for current V2 transaction identity |
